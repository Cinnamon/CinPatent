...
from deepxml.evaluation import get_mirco_f1
...
def main(results, targets, train_labels, a, b):
    ...
    # np.savetxt(res, 'pred.txt')
    # np.savetxt(targets, 'label.txt')

    mlb = MultiLabelBinarizer(sparse_output=True)
    targets = mlb.fit_transform(targets)
    # res = mlb.transform(res).toarray()
    ...
    # print('micro_f1: ', get_mirco_f1(res, targets, mlb))
    # print(metrics.f1_score(targets, res, average='micro'))
