...
class StructuredSelfAttention(BasicModule):
    ...
    def __init__(self, batch_size, lstm_hid_dim, d_a, n_classes, label_embed, w2idx):
        super(StructuredSelfAttention, self).__init__()
        self.n_classes = n_classes
        self.embeddings = torch.nn.Embedding(num_embeddings=len(w2idx), embedding_dim=300)
    ...
    #     return torch.nn.Embedding.from_pretrained(embeddings=embeddings, freeze=False)

    def load_labelembedd(self, label_embed):
        """Load the embeddings based on flag"""
        embed = torch.nn.Embedding.from_pretrained(embeddings=label_embed, freeze=False)
        return embed

    def init_hidden(self):
        h = torch.zeros((2, self.batch_size, self.lstm_hid_dim))
        c = torch.zeros_like(h)

        torch.nn.init.xavier_normal_(h), torch.nn.init.xavier_normal_(c)

        return h.to(device=torch.device('cuda')), c.to(device=torch.device('cuda'))
        # return (torch.randn(2, self.batch_size, self.lstm_hid_dim, device=torch.device("cuda")), torch.randn(2, self.batch_size, self.lstm_hid_dim, device=torch.device("cuda")))
        # return (torch.randn(2,self.batch_size,self.lstm_hid_dim).cuda(),torch.randn(2,self.batch_size,self.lstm_hid_dim).cuda())
        # return (torch.randn(2,self.batch_size,self.lstm_hid_dim),torch.randn(2,self.batch_size,self.lstm_hid_dim))
