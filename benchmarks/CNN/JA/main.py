...
def main():
    ...
    parser.add_argument('--vocab_file', type=str, default='w2idx.json', help='path to word embedding file')
    ...
    if args.do_train:
        ...
        train_loader, val_loader, num_classes, w2idx = dataset(
        ...
        model, criterion, optimizer, scheduler, epochs = initialize_model(
            w2idx=w2idx, embed_dim=args.embed_dim,
            device=DEVICE, 
            train_iter_len=len(train_loader),
            epochs=EPOCHS, 
            lr=LR, p=0.2, 
    ...
    if args.do_predict:
        ...
        _, test_loader, _, w2idx = dataset(train_path=TRAIN_PATH, val_path=TEST_PATH, vocab_file=VOCAB_FILE, batch_size=batch_size, max_seq_len=max_seq_len)
