...
def save_file(texts: List[str], labels: List[List[str]], root_dir: str, is_train: bool = True):
    if is_train:
        with open(os.path.join(root_dir, 'train_org.txt'), 'w', encoding='utf-8') as f:
            for idx, (text, label) in enumerate(zip(texts, labels)):
                f.write(str(idx) + '\t' + ' '.join(label) + '\t' + text + '\n')
    else:
        with open(os.path.join(root_dir, 'test.txt'), 'w', encoding='utf-8') as f:
            for idx, (text, label) in enumerate(zip(texts, labels)):
                f.write(str(idx) + '\t' + ' '.join(label) + '\t' + text + '\n')
...
def main():
    ...
    save_file(texts=train_texts, labels=train_labels, root_dir=args.storage_dir, is_train=True)
    save_file(texts=test_texts, labels=test_labels, root_dir=args.storage_dir, is_train=False)
    ...
    if val is not None:
        ...
        save_file(texts=val_texts, labels=val_labels, root_dir=args.storage_dir, is_train=False)
