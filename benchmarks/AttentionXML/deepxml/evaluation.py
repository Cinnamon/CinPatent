...
def get_mirco_f1(prediction: TPredict, targets: TTarget, mlb: TMlb = None, classes: TClass = None):
    mlb = get_mlb(classes, mlb, targets)
    if not isinstance(targets, csr_matrix):
        targets = mlb.transform(targets)
    prediction = mlb.transform(prediction)
    return metrics.f1_score(targets, prediction, average='micro')

# get_m_f1 = partial(get_mirco_f1)
