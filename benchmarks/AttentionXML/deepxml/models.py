...
class Model(object):
    ...
    # def predict_step_(self, data_x: torch.Tensor):
    #     self.model.eval()
    #     with torch.no_grad():
    ...
    def train(self, train_loader: DataLoader, valid_loader: DataLoader, opt_params: Optional[Mapping] = None,
              nb_epoch=20, step=100, k=5, early=15, verbose=True, swa_warmup=None, **kwargs):
        ...
        for epoch_idx in range(nb_epoch):
            ...
            for i, (train_x, train_y) in enumerate(train_loader, 1):
                ...
                if global_step % step == 0:
                    ...
                    self.swap_swa_params()
                    if verbose:
                        logger.info(F'Epoch: {epoch_idx} - {i * train_loader.batch_size} - train loss: {round(loss, 5)} '
                                    F'P@5: {p5} - nDCG@5: {n5} - early stop: {e}')

    def predict_(self, data_loader: DataLoader):
        self.load_model()
        with torch.no_grad():
            preds, targets = [], []
        ...
        return metrics.f1_score(np.array(targets), np.array(preds).round(), average='micro')

    def predict(self, data_loader: DataLoader, k=13, desc='Predict', **kwargs):
        self.load_model()
