...
def main(data_cnf, model_cnf, mode, tree_id):
    ...
    if mode is None or mode == 'eval':
        ...
        test_y = np.load('./data/EUR-Lex/test_labels.npy', allow_pickle=True)
        test_y = mlb.transform(test_y)
        logger.info(F'Size of Test Set: {len(test_x)}')

        logger.info('Predicting')
        if 'cluster' not in model_cnf:
            # test_tensor = TensorDataset(torch.tensor(test_x, dtype=torch.long), torch.tensor(test_y, dtype=torch.long))
            # t_loader = DataLoader(train_tensor, model_cnf['predict']['batch_size'], num_workers=4)
            test_loader = DataLoader(MultiLabelDataset(test_x), model_cnf['predict']['batch_size'],
                                     num_workers=4)
            t_loader = DataLoader(MultiLabelDataset(test_x, test_y), model_cnf['predict']['batch_size'],
                                     num_workers=4)
            if model is None:
                model = Model(network=AttentionRNN, labels_num=labels_num, model_path=model_path, emb_init=emb_init,
                              **data_cnf['model'], **model_cnf['model'])
            scores, labels = model.predict(test_loader, k=model_cnf['predict'].get('k', labels_num))
            print('Micro-f1', model.predict_(t_loader))
        else:
            if model is None:
                model = FastAttentionXML(labels_num, data_cnf, model_cnf, tree_id)
            print('Micro-f1: ',model.predict_(t_loader))
